...
macro_rules! new_wrapper {
    ( $font:ident, $wr:ident, $path:expr, $width:expr, $align:expr ) => {
        ...
        let mut $wr = figdriver::Wrapper::builder($font, $width)
            .align($align)
            .build();
    };
...
#[test]
fn line_full() {
    new_wrapper!(fnt, wr, "tests/fixtures/test.flf", 8, figdriver::Align::Left);
...
#[test]
fn wrap_align_left() {
    new_wrapper!(fnt, wr, "tests/fixtures/test.flf", 12, figdriver::Align::Left);
    let captured = std::cell::RefCell::new(Vec::new());
    let cb = |lines: &[String]| captured.borrow_mut().push(lines.to_vec());
...
fn wrap_align_center() {
    new_wrapper!(fnt, wr, "tests/fixtures/test.flf", 12, figdriver::Align::Center);
    let captured = std::cell::RefCell::new(Vec::new());
    let cb = |lines: &[String]| captured.borrow_mut().push(lines.to_vec());
    wr.write_line("this is a new test", &cb);
    let lines = captured.borrow();
...
#[test]
fn wrap_align_right() {
    new_wrapper!(fnt, wr, "tests/fixtures/test.flf", 12, figdriver::Align::Right);
    let captured = std::cell::RefCell::new(Vec::new());
    let cb = |lines: &[String]| captured.borrow_mut().push(lines.to_vec());
    wr.write_line("this is a new test", &cb);
    let lines = captured.borrow();
...
#[test]
fn smushing() {
    new_wrapper!(fnt, wr, "fonts/small.flf", 60, figdriver::Align::Left);
...
#[test]
fn kerning() {
    new_wrapper!(fnt, wr, "fonts/small.flf", 60, figdriver::Align::Left, mode figdriver::LayoutMode::Kern);
...
#[test]
fn overlap() {
    new_wrapper!(fnt, wr, "fonts/standard.flf", 60, figdriver::Align::Left, mode figdriver::LayoutMode::Overlap);
...
#[test]
fn full_width() {
    new_wrapper!(fnt, wr, "fonts/small.flf", 60, figdriver::Align::Left, mode figdriver::LayoutMode::FullWidth);
...
#[test]
fn right_to_left() {
    new_wrapper!(fnt, wr, "fonts/small.flf", 60, figdriver::Align::Right, rtl);
...
fn leading_blanks_preserved() {
    ...
    new_wrapper!(fnt, wr, "tests/fixtures/test.flf", 30, figdriver::Align::Left);
    let captured = std::cell::RefCell::new(Vec::new());
    let cb = |lines: &[String]| captured.borrow_mut().push(lines.to_vec());
    wr.write_line("   x", &cb);
    let lines = captured.borrow();
...
fn rtl_consecutive_blanks_collapsed_at_wrap() {
    ...
    new_wrapper!(fnt, wr, "tests/fixtures/test.flf", 8, figdriver::Align::Left, rtl);
    let captured = std::cell::RefCell::new(Vec::new());
    let cb = |lines: &[String]| captured.borrow_mut().push(lines.to_vec());
    wr.write_line("this   is a test", &cb);
    let lines = captured.borrow();
...
fn rtl_leading_blanks_preserved() {
    ...
    new_wrapper!(fnt, wr, "tests/fixtures/test.flf", 30, figdriver::Align::Left, rtl);
    let captured = std::cell::RefCell::new(Vec::new());
    let cb = |lines: &[String]| captured.borrow_mut().push(lines.to_vec());
    wr.write_line("   x", &cb);
    let lines = captured.borrow();
...
fn rtl_blank_after_wrap_discarded() {
    ...
    new_wrapper!(fnt, wr, "tests/fixtures/test.flf", 4, figdriver::Align::Left, rtl);
    let captured = std::cell::RefCell::new(Vec::new());
    let cb = |lines: &[String]| captured.borrow_mut().push(lines.to_vec());
    wr.write_line("this   is a test", &cb);
    let lines = captured.borrow();
...
fn blank_after_wrap_discarded() {
    ...
    new_wrapper!(fnt, wr, "tests/fixtures/test.flf", 4, figdriver::Align::Left);
    let captured = std::cell::RefCell::new(Vec::new());
    let cb = |lines: &[String]| captured.borrow_mut().push(lines.to_vec());
    wr.write_line("this   is a test", &cb);
    let lines = captured.borrow();
...
fn smush_force_with_enable_uses_font_layout() {
    ...
    new_wrapper!(fnt, wr, "fonts/small.flf", 60, figdriver::Align::Left, mode figdriver::LayoutMode::SmushForce);
    assert!(!wr.push_str("Smushy").is_err());
    let output = wr.get();
...
fn smush_force_without_enable_falls_back_to_overlap() {
    ...
    assert!((font.layout & figdriver::SMUSH_ENABLE) == 0);
    new_wrapper!(fnt, wr, "fonts/banner.flf", 60, figdriver::Align::Left, mode figdriver::LayoutMode::SmushForce);
    assert!(!wr.push_str("Hi").is_err());
    let output = wr.get();
    assert!(!output[0].is_empty());
    let smushed_width = output[0].chars().count();
    new_wrapper!(fnt2, wr_full, "fonts/banner.flf", 60, figdriver::Align::Left, mode figdriver::LayoutMode::FullWidth);
...
fn smush_force_overrides_full_width() {
    ...
    let output = wr.get();
    let smushed_width = output[0].chars().count();
    new_wrapper!(fnt2, wr_full, "fonts/small.flf", 60, figdriver::Align::Left, mode figdriver::LayoutMode::FullWidth);
    assert!(!wr_full.push_str("Hi").is_err());
    let full_width = wr_full.get()[0].chars().count();
    assert!(smushed_width < full_width);
}
...
fn smush_force_overrides_kern() {
    ...
    assert!(!wr.push_str("Smushy").is_err());
    let smushed = wr.get();
    new_wrapper!(fnt2, wr_kern, "fonts/small.flf", 60, figdriver::Align::Left, mode figdriver::LayoutMode::Kern);
...
fn flc_all_chars_skipped_produces_no_output() {
    ...
    let mut wr = figdriver::Wrapper::builder(font, 80)
...
fn paragraph_mode_newline_converts_to_space() {
    ...
    new_wrapper!(fnt, wr, "fonts/small.flf", 80, figdriver::Align::Left);
    ...
    new_wrapper!(fnt2, wr2, "fonts/small.flf", 80, figdriver::Align::Left);
