...
- Resolve default layout mode to kerning (`SMUSH_KERN`) when a font has `old_layout=0` and no explicit layout field
...
- Removed a merge conflict from the README file

## [0.5.0] - 2026-06-12

### Changed

- More API adjustments for FFI compatibility and consistency
- `Wrapper::new()` takes a `FIGfont` directly instead of a `Smusher`
- Alignment passed via builder method instead of `Wrapper::new` arguments
- `Smusher::push_str()` and `push_codes()` no longer return rendered subset

## [0.4.0] - 2026-06-12

### Changed

- Changed API to use owned values instead of borrowed references for FFI compatibility
